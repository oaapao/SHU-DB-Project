...
class Reader(models.Model):
    ...
    photo = models.ImageField(upload_to='imgs', verbose_name='用户头像', null=True, blank=True)
    ...
    def get_borrow_num(self):
        return self.bookslips_set.filter(~Q(status='1')).count()
...
class CIP(models.Model):
    ...
    clazz = models.CharField(max_length=100, verbose_name='图书种类', choices=TYPE_CHOICE, default='5', null=True)
    cover = models.ImageField(upload_to='covers', verbose_name='封面', null=True, blank=True)

    def get_rest_num(self):
        return self.books_set.filter(state='2').count()

    def get_readonly_num(self):
        return self.books_set.filter(place='1').count()
...
class Bookslips(models.Model):
    ...
    restore = models.DateField(null=True, verbose_name='实际归还时间',blank=True)
    status = models.CharField(max_length=5, choices=STATUS_CHOICES, default='0', verbose_name='当前状态')
    ...
    def get_money(self):
        List = Fines.objects.filter(borrow__id=self.id)
        if List.first():
            return List.first().money
        return 0
...
class Bookcollections(models.Model):
    ...
    def __str__(self):
        return '{} {}'.format(self.id, self.isbn)
...
class Bookcart(models.Model):
    ...
    isbn = models.ForeignKey(CIP, on_delete=models.CASCADE, verbose_name='图书ISBN编号')
    reader = models.ForeignKey(Reader, on_delete=models.CASCADE, verbose_name='读者ID')
    status = models.CharField(max_length=5, choices=STATUS_CHOICES, default='未完成', verbose_name='状态')
